...
def execute_on_db(db: Any, query: str, params: tuple[Any, ...] = (), want_lastrowid: bool = False) -> int:
    ...
    if USE_POSTGRES:
        ...
        is_insert = bool(re.match(r"(?is)^\s*insert\s+into\s+\w+", query_for_exec))
        if want_lastrowid and is_insert and " returning " not in query_for_exec.lower():
            query_for_exec = f"{query_for_exec} RETURNING id"
        with db.cursor(row_factory=dict_row) as cur:
            cur.execute(query_for_exec, params)
            if want_lastrowid and is_insert:
                row = cur.fetchone()
...
- En "ADAPTACIONES Y NOTAS" incluye obligatoriamente:
  - Sustituciones aplicadas por restricciones del centro.
  - Escalados por defecto relevantes.
  - Opciones por material/espacio.
  - Notas de ejecucion para el coach.
  Todo sin romper el estimulo original.
